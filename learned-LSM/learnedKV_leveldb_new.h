...
class LearnedKV {
  ...
  int initialize_rocksdb(){
    ...
    options.max_file_size = 16 * 1024; // 16KB // modified in version_set
    options.compression = leveldb::CompressionType::kNoCompression;
    // options.block_cache = leveldb::NewLRUCache(64 * 1024); // 64KB
  ...
  int put(unsigned long long key, uint64_t keySize, string value, uint64_t valueSize){
    ...
    leveldb::Status status = db->Put(leveldb::WriteOptions(), key_str, ptr);
    if (!status.ok()) {
  ...
  int get(unsigned long long key, uint64_t keySize, string& value, uint64_t valueSize){
    ...
    if (filter.possiblyContains(key)){
      ...
      string key_str = ullToString(key);
      leveldb::Status status = db->Get(leveldb::ReadOptions(), key_str, &result);

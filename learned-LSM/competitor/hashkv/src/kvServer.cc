...
bool KvServer::getValue(char *key, len_t keySize, char *&value, len_t &valueSize, bool timed) {
    ...
    static int num_get_from_buffer = 0;
    // get value using the location
    ret = (_valueManager->getValueFromBuffer(key, value, valueSize));
    ...
    if (ret) {
        ...
        num_get_from_buffer++;
        if(num_get_from_buffer % 10 == 0)
            // std::cout<<"num_get_from_buffer: "<<num_get_from_buffer<<std::endl;
        return ret;
    }
    ...
    struct timeval startTime1, endTime;
    ...
    totalDuration += (endTime.tv_sec - startTime1.tv_sec) * 1000000 + (endTime.tv_usec - startTime1.tv_usec);
    count++;
    ...
    // if (count % 100000 == 0) {
    //     // double averageTime = static_cast<double>(totalDuration) / count;

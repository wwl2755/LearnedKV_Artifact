...
class LearnedKV {
  ...
  void bulk_load (vector<pair<string, unsigned long long>> &workloads , unsigned int num_kv){
    ...
    for (int i=0;i<num_kv;i++){
      ...
      if(workloads[i].second == 9736846280066027119ULL){
        cout<<"key: "<<workloads[i].second<<endl;
  ...
  int destroy_rocksdb(){
    // delete bt;
    delete alex;
    int res = std::remove("alex_disk");
    std::remove("alex_disk_data");
  ...
  int get(unsigned long long key, uint64_t keySize, string& value, uint64_t valueSize){
    ...
    unsigned long long pos = alex->Find(key);

...
//     snprintf(time_string, sizeof(time_string), "%s.%03ld", time_string, milliseconds);
...
std::string getTime() {
    ...
    gettimeofday(&tv, NULL);
    ptm = localtime(&tv.tv_sec);
    strftime(date_string, sizeof(date_string), "%H:%M:%S", ptm);
    
    long milliseconds = tv.tv_usec / 1000;
    snprintf(time_string, sizeof(time_string), "%s.%03ld", date_string, milliseconds);
